  ...
  const toggleBookmarkAndSave = async () => {
    try {
      ...
      if (!isArticleBookmarked) {
        ...
        toggleBookmark(true);
        console.log("Article is sauvegarder");
      } else {
    ...
    <>
      <View className="w-full flex-row justify-between items-center px-4 pt-10 pb-4 bg-white">
        ...
        <View className="space-x-3 rounded-full items-center justify-center flex-row">
          ...
          >
            {/* Here we change BookmarkSquareIcon to a heart icon */}
            <FontAwesome
              name={isBookmarked ? "heart" : "heart-o"} // "heart" is filled, "heart-o" is outlined
              size={25} // 2.5 times the standard size you specified
